...
const ExitTempSaveModal = ({ isOpen, onConfirm, onCancel }: ExitTempSaveModalProps) => {
  ...
  return (
    ...
    >
      <span className="self-stretch text-title1 font-bold text-text-04 mb:text-title2">저장하지 않고 나가시겠어요?</span>
      <p className="self-stretch text-subtitle text-text-03 mb:text-body">페이지를 떠날 경우 입력하신 정보가 모두 사라져요.</p>
      <div className="flex items-center justify-start gap-3 flex-nowrap self-stretch pt-2 mb:pt-2 mb:gap-2">
        ...
        >
          <span className="text-subtitle font-medium text-text-04 leading-[30px] tb:text-title2 mb:text-body">나가기</span>
        </Button>
        <Button
          variant="primary"
          size="lg"
